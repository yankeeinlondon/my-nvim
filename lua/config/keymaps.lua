...
if ok_builtin then
  ...
  if ok_themes then
    ...
    nnoremap("<leader>ss", function()
      builtin.spell_suggest(themes.get_dropdown({ previewer = false }))
    end, { desc = "[S]earch [S]pelling suggestions" })
  end
end

-- LSP Keybinds (exported for lsp.lua) ---------------------------
M.map_lsp_keybinds = function(buffer_number)
  nnoremap("<leader>rn", vim.lsp.buf.rename,        { desc = "LSP: [R]e[n]ame",        buffer = buffer_number })
  nnoremap("<leader>ca", vim.lsp.buf.code_action,   { desc = "LSP: [C]ode [A]ction",   buffer = buffer_number })
  nnoremap("gd",         vim.lsp.buf.definition,    { desc = "LSP: [G]oto [D]efn",     buffer = buffer_number })

  -- Telescope-based LSP pickers (only if Telescope is available)
  if ok_builtin then
    ...
    nnoremap("<leader>bs", builtin.lsp_document_symbols,
      { desc = "LSP: [B]uffer [S]ymbols",        buffer = buffer_number })

    nnoremap("<leader>ps", builtin.lsp_workspace_symbols,
      { desc = "LSP: [P]roject [S]ymbols",       buffer = buffer_number })
  end

  -- Non-Telescope LSP keymaps (always safe)
  nnoremap("K",        vim.lsp.buf.hover,           { desc = "LSP: Hover",            buffer = buffer_number })
  nnoremap("<leader>k",vim.lsp.buf.signature_help,  { desc = "LSP: Signature",        buffer = buffer_number })
  inoremap("<C-k>",    vim.lsp.buf.signature_help,  { desc = "LSP: Signature",        buffer = buffer_number })
